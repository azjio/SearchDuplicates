...
If OpenWindow(#Window, 0, 0, w, h, "Search duplicates", #PB_Window_SystemMenu | #PB_Window_SizeGadget | #PB_Window_MaximizeGadget | #PB_Window_MinimizeGadget | #PB_Window_ScreenCentered)
	...
	ListIconGadget(#ListIcon1, 3, 5, w - 6, 200, "Имя", 120, ListIconStyle)
	AddGadgetColumn(#ListIcon1, 1 , "Уровень" , 30)
	AddGadgetColumn(#ListIcon1, 2 , "Путь" , 400)
	; ButtonGadget(#BtnINI, 550, 220, 28, 28, "ini")
	ButtonImageGadget(#BtnAddFile, 50, 220, 28, 28, ImageID(6))
	GadgetToolTip(#BtnAddFile, "Добавить файл")
	ButtonImageGadget(#BtnAddFolder, 10, 220, 28, 28, ImageID(5))
	GadgetToolTip(#BtnAddFolder, "Добавить папку")
	ButtonImageGadget(#BtnAddCSV, 90, 220, 28, 28, ImageID(7))
	GadgetToolTip(#BtnAddCSV, "Добавить список CSV")
	ButtonImageGadget(#BtnSaveCSV, 130, 220, 28, 28, ImageID(8))
	GadgetToolTip(#BtnSaveCSV, "Сохранить в список CSV")
...
; 	HideGadget(#BtnSaveCSV, 1)	  ; отключен так как не прописано действие
	...
	CheckBoxGadget(#ChAutoSel, 170, 220, 90, 28, "Все")
	GadgetToolTip(#ChAutoSel, "Выделить")
; 	HideGadget(#ChAutoSel, #True)

	ButtonImageGadget(#BtnSearch, 550, 220, 28, 28, ImageID(1))
	GadgetToolTip(#BtnSearch, "Начать поиск дубликатов")
	ButtonImageGadget(#BtnDelete, 510, 220, 28, 28, ImageID(2))
	GadgetToolTip(#BtnDelete, "Удалить дубликаты отмеченные флажком")
	ButtonImageGadget(#BtnClear, 470, 220, 28, 28, ImageID(3))
	GadgetToolTip(#BtnClear, "Очистить оба списка")
	ButtonImageGadget(#BtnSet, 430, 220, 28, 28, ImageID(4))
	GadgetToolTip(#BtnSet, "Маска")
...
; 	HideGadget(#BtnSet, 1)	  ; отключен так как не прописано действие
	...
	ListIconGadget(#ListIcon2, 3, 5, w - 6, 200, "Найдено", 500, ListIconStyle | #PB_ListIcon_CheckBoxes | #PB_ListIcon_ThreeState)
...
; 	TextGadget(#StatusBar, 4, 220, 420, 28, "AZJIO   2021.08.11", #PB_Text_Border)
	...
	If CreatePopupImageMenu(#Menu1) ; Создаёт всплывающее меню
		MenuItem(#mDelElem, "Убрать из списка", ImageID(2))
		MenuBar()
		MenuItem(#mLevelm2, "-2 - Полный запрет удаления", ImageID(11))
		MenuItem(#mLevelm1, "-1 - Не будет отмечаться галочкой удаления", ImageID(10))
		MenuItem(#mLevel0, "0 - Удаление в последнюю очередь", ImageID(9))
		MenuItem(#mLevel1, "1 - Уровень удаления", ImageID(9))
		MenuItem(#mLevel2, "2 - Удаление в первую очередь", ImageID(9))
		MenuItem(#mLevelSet, "Задать уровень", ImageID(9))
	EndIf
	If CreatePopupImageMenu(#Menu2) ; Создаёт всплывающее меню
		MenuItem(#mOpenFo, "Открыть расположение", ImageID(12))
		MenuItem(#mOpenFi, "Открыть файл", ImageID(14))
		MenuItem(#mSaveRes, "Сохранить результаты", ImageID(8))
	EndIf
	; 	Select EventType()
		...
		Select WaitWindowEvent()
			...
			Case #PB_Event_Menu
				...
				Select event1
					...
					Case #mLevelSet
						i = GetGadgetState(#ListIcon1)
						If i <> -1
							tmp$ = InputRequester("Задать уровень", "Число от 0 и выше", "")
							If IsDigital(@tmp$)
			...
			Case #PB_Event_Gadget
				...
				Select EventGadget()
					...
					Case #BtnSearch
; 						Compare(2)
						CreateThread(@Compare(), 2)
					Case #BtnDelete
						DeleteFileList2()
					Case #BtnAddFile
						tmp$ = OpenFileRequester("Открыть файл", "", "Все (*.*)|*.*", 0, #PB_Requester_MultiSelection)
						If Asc(tmp$)
							tmp2$ = ""
							While tmp$
								tmp2$ + tmp$ + Chr(10)
						...
						EndIf
					Case #BtnAddFolder
						tmp$ = PathRequester("Открыть", GetCurrentDirectory())
						If Asc(tmp$)
							tmp$ = RTrim(tmp$, #PS$)
							AddList(tmp$, 1)
						EndIf
					Case #BtnSaveCSV
						If CountGadgetItems(#ListIcon2) And MessageRequester("Сохранить CSV из результата?", "Да - Сохранить из результатов поиска (быстро, но дубликаты)" + #LF$ + "Нет - заново вычислить для всех файлов верхнего списка", #PB_MessageRequester_YesNo) = #PB_MessageRequester_Yes
							SaveCSV()
						Else
							SaveCSV2()
						EndIf
					Case #BtnAddCSV
						tmp$ = OpenFileRequester("Открыть файл", "", "Список (*.csv)|*.csv", 0)
						If Asc(tmp$)
							AddList(tmp$, 3)
						EndIf
					Case #BtnSet
						MaskMem$ = InputRequester("Задать маску", "Перечисление png;gif или маску *.jpg*", MaskMem$)
						ClearList(mask())
						If FindString(MaskMem$, ";")
							If FindString(MaskMem$, "*")
								MessageRequester("Возможная ошибка",  "Фильтр не должен содержать элементы маски. Фильтр это перечисление расширений, а маска это wildcard.")
							EndIf
							SplitL(MaskMem$, mask(), ";")
						Else
							If AddElement(mask())
...
Procedure SaveRes()
	...
	If Not CountLV
	    ProcedureReturn
	EndIf
	File$ = SaveFileRequester("Сохранить", "Список дубликатов.txt", "Текст (*.txt)|*.txt", 0)
	If Asc(File$)
		If Right(File$, 4) <> ".txt"
			File$ + ".txt"
		EndIf
...
Procedure SaveCSV()
	...
	If Not CountLV
	    ProcedureReturn
	EndIf
	File$ = SaveFileRequester("Сохранить", "", "Текст (*.csv)|*.csv", 0)
	If Asc(File$)
		If Right(File$, 4) <> ".csv"
			File$ + ".csv"
		EndIf
	...
	ForEach items()
		...
		Else
			If LCase(GetExtensionPart(items())) = "csv"
				If type = 3 Or MessageRequester("CSV?", "Открыть файл как CSV?", #PB_MessageRequester_YesNo) = #PB_MessageRequester_Yes
...
Procedure DeleteFileList2()
	...
	If Asc(Error$)
		If CountString(Error$, #LF$) > 20
			...
			MessageRequester("Ошибки удаления", "Ошибок удаления больше 20, поэтому скопированы в буфер обмена")
		Else
			MessageRequester("Ошибки удаления", RTrim(Error$, #LF$))
		EndIf
	EndIf
EndProcedure
...
Procedure Compare(*Valie)
	...
	Protected StartTime, StartTimeNew
	...
	StatusBarText(#StatusBar, 0, "Поиск файлов")
...
; 			Next
	...
	If tsz > 100000000
		...
		Next
		If MessageRequester("Продолжить?", "Вычислить MD5 для " + _ShortFileSize(tsz) + "?" + #LF$ + #LF$ +  "Топовые файлы по размеру:" + #LF$ + tmp$, #PB_MessageRequester_YesNo) = #PB_MessageRequester_No
			FreeMap(DataFSL())
			ProcedureReturn
		EndIf
	EndIf
...
; 	Перебираем списки одного размера, получая md5
	ForEach DataFSL()
		...
		If ListSize(DataFSL()\Lst1()) <> 1
			ForEach DataFSL()\Lst1()
				StartTimeNew = ElapsedMilliseconds()
				If StartTimeNew - StartTime > 300
...
; 	Сортировка групп по размеру
	...
	StatusBarText(#StatusBar, 0, "Подготовка вывода результатов")
...
; 	StatusBarText(#StatusBar, 0, "Завершено! Групп: " + Str(k) + ", файлов: " + LSize$ + ", дубликатов: " + Str(k+ch) + ", отмеченных: " + Str(ch))
; 	StatusBarText(#StatusBar, 0, "Завершено! Групп: " + Str(k) + ", дубликатов: " + Str(k+ch) + ", отмеченных: " + Str(ch))
	StatusBarText(#StatusBar, 0, "Завершено! Групп: " + Str(k) + ", дубликатов: " + Str(idx - k + 1) + ", отмеченных: " + Str(ch) + ", выполнено за " + FormTime(TimeCompare))
...
; 				FileSearch(DataFSL(), item_txt$, "*", 130, level, i)
